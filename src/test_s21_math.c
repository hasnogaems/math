...
START_TEST(pow_test_1) {
  for (double k = -9999; k <= 9999; k += 1.7) {
    for (double g = -555; g < 555; g += 1) {
...
START_TEST(log_test_3) {
  for (double k = 0.1; k < 10000; k += 123.24) {
    double a = s21_log(k);
    double b = log(k);
    ck_assert_double_eq_tol(a, b, 1e-6);
  }
